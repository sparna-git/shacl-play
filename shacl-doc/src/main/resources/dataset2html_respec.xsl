<xsl:stylesheet version="2.0"
	...
	<xsl:import href="doc2html_respec.xsl"/>
	...
	<xsl:template match="ShapesDocumentation" mode="style_css_extra">		
		...
		.table-striped_dataset tr:nth-child(even) {
	...
	<!-- show number of targets as badge -->
	...
	<xsl:template match="propertyGroups">
		<xsl:if test="count(propertyGroup) > 0">
			<table class="sp_table_propertyshapes table-striped_dataset table-responsive">
				<thead>
					<tr>
						<th>
							<xsl:value-of
	...
	<xsl:template match="chart">
		...
		<div class="chart" id="{$currentSectionId}">
			<h5>
				<xsl:value-of select="../../title" /> <xsl:value-of select="$LABELS/labels/entry[@key='BY']/@label" />
