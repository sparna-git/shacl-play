...
public class ComputeStatisticsVisitor extends DatasetAwareShaclVisitorBase implements ShaclVisitorIfc {
	...
	public void visitNodeShape(Resource aNodeShape) {
		...
		if(!targetDefinition.isEmptyTarget()) {
			int count = this.statisticsProvider.countTargets(targetDefinition);
			...
			log.warn("Found node shape without target definition '{}', cannot compute statistics", aNodeShape.getURI());
		}
	}
	...
	public void visitPropertyShape(Resource aPropertyShape, Resource aNodeShape) {
		...
		if(!targetDefinition.isEmptyTarget()) {
			...
			// count number of distinct objects, only if present
			int countDistinctObjects = 0;
			if(count > 0) {
				countDistinctObjects = this.statisticsProvider.countDistinctObjects(
					targetDefinition,
					propertyPath
				);
			}
			...
			log.warn("Cannot count occurrences and distinct values inside node shape without target '{}', for property shape '{}'", aNodeShape.getURI(), aPropertyShape.getURI());
		}
	}
