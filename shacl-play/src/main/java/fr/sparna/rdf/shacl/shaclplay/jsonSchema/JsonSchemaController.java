...
public class JsonSchemaController {
	...
	public ModelAndView schema(HttpServletRequest request, HttpServletResponse response) {
		JsonSchemaFormData data = new JsonSchemaFormData();

		return new ModelAndView("jsonschema-form", JsonSchemaFormData.KEY, data);
	}

	@RequestMapping(value = { "jsonschema" }, params = { "url" }, method = RequestMethod.GET)
	public ModelAndView schema(@RequestParam(value = "url", required = true) String shapesUrl,
			// URL Option
			@RequestParam(value = "IdUrl", required = false) String urlRoot, HttpServletRequest request,
	...
	@RequestMapping(value = "/jsonschema", params = { "shapesSource" }, method = RequestMethod.POST)
	public ModelAndView schema(
			...
			@RequestParam(value = "loadNodeShape", required = false) boolean loadNodeShape, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		try {

			// get the source type
			ControllerModelFactory.SOURCE_TYPE shapesSource = ControllerModelFactory.SOURCE_TYPE
					.valueOf(shapesSourceString.toUpperCase());

			if (loadNodeShape) {

				// Read model input
				Model shapesModel = ModelFactory.createDefaultModel();
				ControllerModelFactory modelPopulator = new ControllerModelFactory(
						this.catalogService.getShapesCatalog());
				modelPopulator.populateModel(shapesModel, shapesSource, null, // shapesUrl,
						null, // shapesText,
						shapesFiles, null // shapesCatalogId
				);

				// Generate
				List<NodeShape> ns = new ShapesGraph(shapesModel, null).getAllNodeShapes();
				List<String> rootUris = ns.stream().map(n -> n.getNodeShape().getURI()).collect(Collectors.toList());
				
				request.getParameter("rootURISNodeShapes");
				request.setAttribute("uris", rootUris);
				
				//Object q = request.getParameter("rootURISNodeShapes");
				//System.out.println(q.toString());
				
				//ModelAndView modelmv = new ModelAndView("rootURISNodeShapes");
				//modelmv.addObject("uris", rootUris);
				//return new ModelAndView("jsonschema-form", JsonSchemaFormData.KEY, "uris");
	...
	private Model doSchemaShapes(Model shapesModel, String rootUri, HttpServletResponse response) throws Exception {
	 ...
	 * @return
	 */
	protected ModelAndView handleGenerateFormError(HttpServletRequest request, String message, Exception e) {
		JsonSchemaFormData data = new JsonSchemaFormData();
