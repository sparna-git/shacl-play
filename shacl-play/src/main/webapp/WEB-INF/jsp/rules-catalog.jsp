...
<c:set var="data" value="${requestScope['RulesCatalogData']}" />
...
<body>
	<jsp:include page="navbar.jsp">
		<jsp:param name="active" value="rules-catalog"/>
	</jsp:include>
    ...
    <div class="container-fluid">
    	...
    	<div class="row justify-content-md-center">
            <div class="col-8">
            	<h1 class="display-3"><fmt:message key="catalog-rules.title" /></h1>
            		<div class="row">
				        	...
				        	<div class="col-4 shapes-card-container">
			            		<div class="card" id="eli-shapes">
						          ...
						          <div class="card-footer text-muted">
									 <p class="card-text catalog-user">&nbsp;<c:if test="${not empty entry.submitted}"><fmt:message key="catalog-rules.entry.addedOn" /> <fmt:formatDate type="date" value = "${entry.submitted}" /> <c:if test="${not empty entry.submitter.label}"><fmt:message key="catalog-rules.entry.addedOn.by" /> ${entry.submitter.label}</c:if></c:if></p>	
								  </div>
						        </div>
					        </div>
				      	</c:forEach>
